...
5. The server should now be fully operational. To install the server as a Windows service, run `node windowsServiceSetup.js` within the server directory. Make sure that MySQL Server is running before doing this. To check that both the server and MySQL services are running, press Windows+R to open the run prompt, and enter services.msc. This will open the Windows Services Manager. Find a service named something like MySQL80 and confirm that it is running. If it is not, start it. Next, find the service called TU-Relay Server. Again, start it if it is not running. The server should now be accessible even if you log out.
